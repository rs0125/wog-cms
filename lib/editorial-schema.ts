...
// (website src/pages/EditorialLocationPage.tsx), so one schema validates all
// three; ./micromarket-schema.ts and ./location-schema.ts add only the fields
// that address a page.
//
// What these tables hold is prose and nothing else. Every figure on the page —
// listing count, rent and size ranges, construction mix, compliance counts, the
// peer rent chart — is derived from live inventory by the backend, which is what
// stops copy written today from contradicting the grid on the same page a year
// from now.
 ...
 */
export const optionalProse = nonEmpty.nullable();


/**
 * The content fields every scope shares. Spread into a concrete schema next to
 * the fields that address the page.
 */
...
export type EditorialFaq = z.infer<typeof faqSchema>;
