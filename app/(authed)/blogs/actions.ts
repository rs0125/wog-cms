...
/** `listed` is the state the blog ended up in; `at` distinguishes one toggle from the next. */
...
export async function triggerSiteBuild(): Promise<string | undefined> {
  ...
  try {
    ...
    return `${started} But the CMS could not record it, so blogs may still show as Staged.`;
  }
